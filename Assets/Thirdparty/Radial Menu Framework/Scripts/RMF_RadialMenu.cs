...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    [SerializeField]
    Transform m_ChildSpawnTransform;
    [SerializeField]
    ...
    void Awake() {
        WeaponsSingleton.Instance.AddItemToInventory += addRMFButton;
        WeaponsSingleton.Instance.DropItemFromInventory += Instance_DropItemFromInventory;
    ...
    private void Instance_DropItemFromInventory(Sprite arg1, string arg2)
    {
        var k = elements.Find(x => x.label == arg2);
        elements.Remove(k);
    ...
    void Update() {
        ...
        if (angleOffset != 0 && useLazySelection) {
            ...
            if (elements.Count > 0 && elements[index] != null) {
    ...
    void addRMFButton(Sprite sprite, string label)
    {
        var j = Instantiate(m_ButtonPrefab, m_ChildSpawnTransform);
        j.GetComponent<RMF_RadialMenuElement>().SetupSprite(sprite, label);
        elements.Add(j.GetComponent<RMF_RadialMenuElement>());
        performReset(false);
    }
    ...
    void performReset(bool reset)
    {
        if (reset)
        {
            ...
            for (int i = 0; i < elements.Count; i++)
            {
                ...
                RectTransform elemRt = elements[i].GetComponent<RectTransform>();
        ...
        for (int i = 0; i < elements.Count; i++)
        {
            ...
            RectTransform elemRt = elements[i].GetComponent<RectTransform>();
            elemRt.rotation = Quaternion.Euler(0, 0, -((360f / (float)elements.Count) * i) - globalOffset);
